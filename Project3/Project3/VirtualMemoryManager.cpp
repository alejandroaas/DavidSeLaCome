...
*/
VirtualMemoryManager::VirtualMemoryManager(ReplacementPolicy p, unsigned int pS, unsigned int nF, unsigned int vA) : virtualMemoryManagerInterface(p,  pS, nF, vA)
{
...
*/
unsigned long long VirtualMemoryManager::memoryAccess(unsigned long long address){
...
Output: Physical Address
*/
unsigned long long VirtualMemoryManager::LRUPolicy(unsigned long long address){
	//Variables init
...
Output: Physical Address
*/
unsigned long long VirtualMemoryManager::FIFOPolicy(unsigned long long address){
	//Variables init
	unsigned long long vpn = address >> (N/2); // Virtual page number --> key of pageTable map
	unsigned long long ppn = -1; // Physical page number --> key of memory map 
	unsigned long long offset = address % vpn;
...
Output: frame
*/
frame VirtualMemoryManager::buildFrame(unsigned long long offset){
...
*/
std::map<unsigned long long, frame>::iterator VirtualMemoryManager::findLRU(){
	std::map<unsigned long long, frame>::iterator memoryIter, lru;
	lru = memory.begin();
...
Output: Memory map iterator
*/
std::map<unsigned long long, frame>::iterator VirtualMemoryManager::findFIFO(){
	std::map<unsigned long long, frame>::iterator memoryIter, fifo;
	fifo = memory.begin();
	for (memoryIter = memory.begin(); memoryIter != memory.end(); memoryIter++){
